================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: JSOpen プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した JSOpen には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには JSOpen アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
JSOpen.h
   ...
   CJSOpenApp アプリケーション クラスを宣言します。

JSOpen.cpp
   これは、アプリケーション クラス CJSOpenApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\JSOpen.ico
   ...
   アイコンはメイン リソース ファイル JSOpen.rc に含まれていま
   す。
...
JSOpenDoc.h, JSOpenDoc.cpp - ドキュメント
   これらのファイルは CJSOpenDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CJSOpenDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。

JSOpenView.h, JSOpenView.cpp - ドキュメントのビュー
   これらのファイルは CJSOpenView クラスを含みます。
   CJSOpenView オブジェクトは CJSOpenDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   JSOpen.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
